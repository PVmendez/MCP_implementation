...
export class MCPService {
        ...
        this.tools = [
            {
                ...
                execute: async (params) => {
                    ...
                    const todoItem = await this.todoItemsService.create(list.id.toString(), { description: params.description });
                    return { message: `Item "${params.description}" creado en la lista "${params.list_name}"`, item: todoItem };
                },
            },
            {
                ...
                execute: async (params) => {
                    const todoItem = await this.todoItemsService.update(params.item_id, { completed: true });
                    return { message: `Item "${todoItem.description}" marcado como completado`, item: todoItem };
                },
            },
            {
                ...
                execute: async (params) => {
                    const todoItem = await this.todoItemsService.update(params.item_id, { description: params.description });
                    return { message: `Item actualizado a "${params.description}"`, item: todoItem };
                },
            },
            {
    ...
    async executeTool(toolName: string, params: Record<string, any>): Promise<any> {
        ...
        if (missingParams.length > 0) {
            throw new Error(`Faltan los siguientes parámetros requeridos: ${missingParams.join(', ')}`);
        }
